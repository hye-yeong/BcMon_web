...
let dataSource=[];
let url = [
  "http://bcmon.kmu.ac.kr/coin-data/bitcoin/graphs/network/tcp-segment/day",
  "http://bcmon.kmu.ac.kr/coin-data/bitcoin/graphs/network/tcp-segment/year"
];
var in_value = "in_value";
var out_value = "out_value"
var title = "TCP Segment";
var caption = "The total number of segments received / sent, including those received in error.";

ReactFC.fcRoot(FusionCharts, charts, Zoomline, FusionTheme);
charts(FusionCharts);

class tcpSegment_sum extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      bigChartData: "0"
            ...
            <Col xm="12" md={{ size: 10, offset: 1 }}>
              <Card>
                ...
                <CardBody>
                    ...
                    tbody={[
                      {
                        data: [
                          ...
                          { text: "SNMP tcpInSegs(수신된 세그먼트의 수), tcpOutSeg(송신된 세그먼터의 총 개수)" },
                        ]
                      },
                      {
                        data: [
                          { className: "text-center", text: "수집 방법" },
                          { text: "비트코인 노드가 설치된 서버의 SNMP interface에서 20초 간격으로 tcpInSegs(수신된 세그먼트의 수), tcpOutSeg(송신된 세그먼터의 총 개수) data 수집" },
                        ]
                      },
                      {
                        data: [
                          { className: "text-center", text: "분석 방법" },
                          { text: "당일 tcpInSegs 총합, 당일 tcpOutSeg 총합 " },
                        ]
                      },
                    ]}
                  />
                  ...
                  <ul>
                    <li>tcpInSegs:오류로 수신된 것을 포함하여 수신된 세그먼트의 총 개수</li>
                  </ul>
                  <ul>
                    <li>tcpOutSeg:현재 연결된 것을 포함하여 송신된 세그먼트의 총 개수(재전송되는 옥테트를 제외)</li>
                  </ul>
                  <ul>
                    <li>TCP(Transmission COntrol PRotocol): 인터넷상에서 데이터를 메세지의 형태로 보내기 위해 IP와 함께 사용하는 프로토콜</li>
                  </ul>
                </CardBody>
              </Card>
            </Col>
